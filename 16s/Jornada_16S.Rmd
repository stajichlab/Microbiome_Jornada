...
ps.ord <- ordinate(ps.prune, "NMDS", distance="bray")
psVegZone = plot_ordination(ps.prune, ps.ord, type = "samples", color = "VegZone") + stat_ellipse(geom = "polygon", alpha = 1/18, aes(fill = VegZone)) + theme_bw() + ggtitle("Beta Diversity (NMDS) by VegZone")
print(psVegZone)

ps.ord.dca <- ordinate(ps.prune, "DCA")
psVegZone = plot_ordination(ps.prune, ps.ord.dca, type = "samples", color = "VegZone") + stat_ellipse(geom = "polygon", alpha = 1/18, aes(fill = VegZone)) + theme_bw() + ggtitle("Beta Diversity (DCA) by VegZone")
print(psVegZone)
ps.ord.rda <- ordinate(ps.prune, "RDA")
psVegZone = plot_ordination(ps.prune, ps.ord.rda, type = "samples", color = "VegZone") + stat_ellipse(geom = "polygon", alpha = 1/18, aes(fill = VegZone)) + theme_bw() + ggtitle("Beta Diversity (RDA) by VegZone")
print(psVegZone)

```
# break down the phylogeny
# still need better taxonomy

``` {r Test for PhyloOTU signals by Env paramter}

# get Proteobacteria
otusProteobacteria = subset_taxa(ps.prune, Phylum == "Proteobacteria")
proteobacteria.ps.ord <- ordinate(otusProteobacteria, "NMDS", distance="bray")
ProteobacteriapsVegZone = plot_ordination(otusProteobacteria, proteobacteria.ps.ord, type = "samples", color = "VegZone") + stat_ellipse(geom = "polygon", alpha = 1/18, aes(fill = VegZone)) + theme_bw() + ggtitle("Beta Diversity (NMDS) by VegZone - Proteobacteria")
print(ProteobacteriapsVegZone)

otusCyano = subset_taxa(ps.prune, Class == "Cyanobacteria")
Cyano.ps.ord <- ordinate(otusCyano, "NMDS", distance="bray")
CyanoVegZone = plot_ordination(otusCyano, Cyano.ps.ord, type = "samples", color = "VegZone") + stat_ellipse(geom = "polygon", alpha = 1/18, aes(fill = VegZone)) + theme_bw() + ggtitle("Beta Diversity (NMDS) by VegZone - Cyano")
print(CyanoVegZone)

otusArchaea = subset_taxa(ps.prune, Kingdom == "Archaea")
Archaea.ps.ord <- ordinate(otusArchaea, "NMDS", distance="bray")
ArchaeaVegZone = plot_ordination(otusArchaea, Archaea.ps.ord, type = "samples", color = "VegZone") + stat_ellipse(geom = "polygon", alpha = 1/18, aes(fill = VegZone)) + theme_bw() + ggtitle("Beta Diversity (NMDS) by VegZone - Archaea")
print(ArchaeaVegZone)
